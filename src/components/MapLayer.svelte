  ...
  map.on('click', id, (e) => {
    ...
    if (items.length == 1) {
      ...
      items.forEach((item) => {
        ...
        let contentTxt = `
            <a class="multipoint" href="/farms/${
              item.properties.recordid
            }?${searchparamswithlocation.toString()}">
                <div class="icon ${item.properties.categorie
                  .replaceAll(' ', '-')
                  .toLowerCase()
                  .normalize()}"></div>
...
<div class="multipoint-mobile-container" bind:this={multipointmobilectn}>
  <div class="close is-clickable p-5">
    <i class="fas fa-times" aria-hidden="true" on:click={closeMobilePopup} />
  </div>
  <div class="content" bind:this={multipointmobilecontent} />
</div>
